...
/**
 * cssrHMM uses a continuous version of CSSR to form a Hidden Markov Model.
 * The process differs from the three phases publishes by Shalizi:<br>
 * <ol>
 * <li><b>Morphs:</b> the frequency counts are tallied for each history of Lmax in the
 * raw data. O(N). Phase I </li>
 * <li><b>Homogenity:</b> the histories are sorted and partitioned into contiguous
 * Suffix Groups with homogenous morphs. O(kL.log(kL)). Phase II</li>
 * <li><b>Suffixes:</b> the morph is caculated for each Suffix Group. O(kL). Phase II.
 * </li>
 * <li><b>Determinist:</b> the subsequent Suffix Group is established for each Suffix
 * Group. O(kl). Phase III.</li>
 * <li><b>States:</b> Suffix Groups which are both homogenous and deterministic are
 * combined and assigned states. O(?). Phase II.</li>
 *
 * @author nworbnhoj
 */
public class cssrHMM extends HMM {
    ...
    /** all of the characters observed from the data source*/
    private static List<Character> alphabet = new ArrayList<Character>();
    /** maximum history Length */
    private int Lmax = 4;
    /** holds the next observation */
    private Character nextObservation = '\u0000';
    /** holds the previous Lmax observations */
    private String recentHistory = "";
    /** the significance test algorithms available */
    enum TestType {

        KS, CHIS
    };
    /** the selected significance test algorithm */
    private static TestType testtype = TestType.CHIS;
    /** the significance set for the null hypothesis */
    private double sigTest = 0.01;
    /* 
     * holds a record of HMM states which have been disrupted and the
     * transitions than need to be remapped.
     */
    private Map<Integer, Set<Character>> unmappedHMMTransitions =
     ...
     * The morph trie is initialized with a root node.
     ...
     * either a change is made to the HMM or the end of the data source is
     * reached.
     */
...
// </editor-fold>
    ...
    /**
     * isolateChild takes a childNode that has been recognized as being non-homogenous
     ...
     */
    private HashMap<Integer, Set<Character>> splitFamily(
            MorphNode parentNode,
            MorphNode childNode) {
     ...
     */
    private void propogateTemplate(
            MorphNode parentNode,
            HashMap<Integer,
            Set<Character>> stateTemplate) {
        if ((parentNode == null) || (stateTemplate == null)) {
            return;
        }
        if (parentNode.getChildren().isEmpty()) {
            return;
        }
        if (parentNode.getId().length() < 1) {
            return;
        }
    ...
    /** 
     * mapTransitions steps  thru the previously recorded disrupted transitions
     * in the HMM graph and re-maps them.
     */
...
// </editor-fold>
     ...
     * stored in the first morph associated with obs
...
//            prob = Stats.KSsig(morph1.getTallies(), morph2.getTallies());
     ...
     * @return a nonHomogeneous MorphNode if any exist.
     */
